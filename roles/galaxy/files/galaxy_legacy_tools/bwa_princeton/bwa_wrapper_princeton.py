...
def __main__():
    ...
    parser.add_option( '-q', '--qualThreshold', dest='qualThreshold', help='Quality threshold for read trimming down to 35bp' )
    ...
    if options.params == 'pre_set':
        ...
        if options.noIterSearch == 'true':
            ...
            noIterSearch = ''
        aligning_cmds = '-n %s -o %s -e %s -d %s -i %s %s -k %s -t %s -M %s -O %s -E %s %s %s %s %s -q %s' % \
                        ( editDist, options.maxGapOpens, options.maxGapExtens, options.disallowLongDel,
                          ...
                          suboptAlign, noIterSearch, color_space, illumina_quals, options.qualThreshold )
        if options.genAlignType == 'paired':
